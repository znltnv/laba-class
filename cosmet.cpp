...
void change1(char namefile[30]){
	...
	Cosmetik cos2;
	Cosmetik cos3;
	cout << "Введите название продукта количество которого хотите изменить "<< endl;
	cin >> cos2.name;
	cout << "Введите количество этого продукта" << endl;
	cin >> cos2.count;
	int j=0;
	for (int i = 0; i < d ; i++) 
	{
	fin >> cos3;
		if (!(cos3.name == cos2.name)){
			...
			j++;
				...
				cos[j].type.assign(cos3.type);
				cos[j].name.assign(cos3.name);
				cos[j].count = cos2.count;
				cos[j].price = cos3.price;
				j++;
				}
	}
	fin.close();
